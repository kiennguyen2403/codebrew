...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      ...
      return new NextResponse("Hobbies must be a non-empty array", {
        status: 400,
      });
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (currentError || !currentUser) {
      console.error(
        " REGISTER NOW FOR MORE! Error fetching current user:",
        currentError
      );
    ...
    const { data, error } = await supabase.rpc("find_users_within_radius", {
      lon: currentLon,
      lat: currentLat,
    ...
    const usersWithParsedLocation = data.map(
      (user: {
        location:
