    ...
    switch (id) {
      ...
      case "recommendation": {
        ...
        if (!currentUser.hobby_embedding || !currentUser.location) {
          return new NextResponse("User missing hobbies or location", {
            status: 400,
          });
        }
        ...
        const geometryCurrent = wkx.Geometry.parse(
          Buffer.from(wkbHexCurrent, "hex")
        );
        const locationCurrent = geometryCurrent.toGeoJSON() as {
          type: string;
          coordinates: number[];
        };
        if (
          !locationCurrent ||
          locationCurrent.type !== "Point" ||
        ...
        const { data: similarUsers, error: searchError } = await supabase.rpc(
          "match_users_by_hobbies",
          {
        ...
        if (fetchError) {
          ...
          return new NextResponse("Failed to fetch recommended users", {
            status: 500,
          });
        }
        ...
        const recommendations = recommendedUsers.map((user: any) => {
          ...
          const userLocation = geometry.toGeoJSON() as {
            type: string;
            coordinates: number[];
          };
          ...
          const distanceQuery = supabase.rpc("calculate_distance", {
            lat1: currentLat,
            lon1: currentLon,
            lat2: lat,
            lon2: lon,
